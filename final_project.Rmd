...
```{r intro, echo = TRUE, message = TRUE, warning = FALSE, results='hide', cache=TRUE, include=FALSE}
...
```{r, head, echo = TRUE, tidy=TRUE, cache=TRUE}
str(movielens)
```

**The first few lines of the dataset look like this:**

```{r, head2, echo = TRUE, tidy=TRUE, cache=TRUE}
head(movielens) %>%
  print.data.frame()
```
\pagebreak
...
```{r movie_summary, include=FALSE, echo = TRUE, tidy=TRUE, cache=TRUE}
summary(movielens)
```

```{r data_summary ,include=FALSE, echo = TRUE, tidy=TRUE, cache=TRUE}
...
```{r data_summary2 , echo = TRUE, tidy=TRUE, cache=TRUE, include=FALSE}
print(data_summary)
```
...
```{r percent_populated, echo = TRUE, cache=TRUE} 
...
```{r populated_graph, echo = TRUE, fig.align='left', cache=TRUE} 
...
```{r user_reviews_range, echo = TRUE, warning=FALSE, message=FALSE, tidy=TRUE, cache=TRUE} 
...
```{r user_density_distribution, echo = TRUE, cache=TRUE} 
users %>%
  ggplot(aes(n)) + 
...
```{r user_avg_rating, echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE} 
g1 <- users %>%
...
```{r movie_histogram, echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE} 
...
```{r rating_grid_showing_trends, echo = TRUE, warning = FALSE, message = FALSE, cache=TRUE}
...
```{r bigger_films, echo = TRUE, message=FALSE, warning=FALSE, cache=TRUE} 
...
```{r ratings_distributions, echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE}
movielens %>%
...
```{r avg_rating_vs_year,  echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE}
movielens %>%
...
```{r genre_separted,  echo = FALSE, warning=FALSE, message=FALSE, tidy=TRUE, cache=TRUE}
#movielens <- movielens %>% separate_rows(genres, sep ="\\|")
#genres <- movielens %>% separate_rows(genres, sep ="\\|") %>% unique(genres)
#print(genres)
#movies_by_genre <- movielens %>% separate_rows(genres, sep ="\\|") %>% group_by(genres) %>% summarize(count = n()) %>% arrange(count)
...
```{r genres_ratings_facet,  echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r reviews_by_genre,  echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE}
#movies_by_genre <- movielens %>% separate_rows(genres, sep ="\\|") %>% 
#  group_by(genres) %>% summarize(count = n()) %>% arrange(count)
...
```{r avg_rating_by_genre_plots,  echo = TRUE, warning=FALSE, message=FALSE, fig.align='left', cache=TRUE}
#ratings_summary_by_genre <- movielens %>% separate_rows(genres, sep ="\\|") %>% 
#      group_by(genres) %>% summarise(avg_rating = mean(rating)) 
...
```{r data_separation_section_1,  echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r rmse_formula,  echo = TRUE, warning=FALSE, cache=TRUE}
RMSE <- function(true_ratings, predicted_ratings){
  sqrt(mean((true_ratings - predicted_ratings)^2))
}
```

```{r test_train_break,  echo = TRUE, warning=FALSE, cache=TRUE}
...
```{r rmse_output_1, echo = TRUE , cache=TRUE}
...
```{r minus_mu_minus_bi,  echo = TRUE, warning=FALSE, message=FALSE , cache=TRUE}
...
```{r minus_mu_minus_bi_minus_bu, echo = TRUE, echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r minus_mu_minus_bi_minus_bu_1, echo = TRUE, echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r lambdas, echo = TRUE,  warning=FALSE, message=FALSE, cache=TRUE}
...
```{r regularized_rmse_3, echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r regularized_movie_and_user, echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r regularized_movie_and_user_and_year, echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r regularized_movie_and_user_and_year_and_genre, echo = TRUE,  warning=FALSE, message=FALSE, cache=TRUE}
...
```{r regularized_movie_and_user_and_year_and_genre_validation, echo = TRUE,  warning=FALSE, message=FALSE, cache=TRUE}
...
```{r regularized_movie_and_user_and_year_validation, echo = TRUE,  warning=FALSE, message=FALSE, cache=TRUE}
