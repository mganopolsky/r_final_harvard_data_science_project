...
library(here)
directory <- here::here()
env_file <- paste(directory, "/env.RData", sep="")
#str(env_file)
load(file = env_file)
...
```{r intro, echo = TRUE, message = TRUE, warning = FALSE, results='hide', cache=TRUE}
...
```{r movie_summary, summary, echo = TRUE, tidy=TRUE, cache=TRUE}
summary(movielens)
```

```{r data_summary , echo = TRUE, tidy=TRUE, cache=TRUE}
...
```{r data_summary2 , echo = TRUE, tidy=TRUE, cache=TRUE}
print(data_summary)
```
...
```{r genre_separted,  echo = TRUE, warning=FALSE, message=FALSE, tidy=TRUE, cache=TRUE}
